...
public class CurrentRunners implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (Main.database.currentRunners.isEmpty()) {
            commandSender.spigot().sendMessage(new ComponentBuilder("No runners on course").color(ChatColor.RED).bold(true).create());
        } else {
            for (Map.Entry<Integer, SportIdent> runner : Main.database.currentRunners.entrySet()) {
                ...
                if (runner.getValue().splits.controls.isEmpty()) {
                    ...
                    Split lastControl = runner.getValue().splits.controls.get(runner.getValue().splits.controls.size() - 1);
                ...
                commandSender.spigot().sendMessage(new ComponentBuilder(
                        ...
                        .bold(true)
                        .underlined(true)
                        .event(new HoverEvent(HoverEvent.Action.SHOW_ENTITY, new ComponentBuilder("{name:\"" + player.getName() + "\", type:\"Player\", id:\"" + player.getUniqueId() + "\"}").create()))
                        .event(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format("/tp @p %s", player.getName())))
                        .create());
            }
        }
        return true;
