        ...
        try {
            if (strings.length > 0) {
                if (strings[0].equals("add")) {
                    ...
                    Main.database.finishedRunners.put(Main.database.currentRunnersID,
                            new Result(Bukkit.getPlayer(strings[1]), strings[2], Result.Status.OK, true));
                    return true;
                }
            }
            ...
            for (Map.Entry<Integer, SportIdent> entry : Main.database.currentRunners.entrySet()) {
                results.add(new Result(entry.getValue().player, entry.getValue().splits.getTotalTime(),
            ...
            for (Result result : results) {
                if (result.place == null) {
                    lines.add(ChatColor.translateAlternateColorCodes('&',
                            String.format("&2[R] %s %s", result.player.getDisplayName(), result.time)));
                } else {
                    lines.add(ChatColor.translateAlternateColorCodes('&',
                            String.format("&l%s %s %s", result.place, result.player.getDisplayName(), result.time)));
                }
            }
            for (Result result : mispunches) {
                lines.add(ChatColor.translateAlternateColorCodes('&',
                        String.format("&l&4%s %s %s", result.place, result.player.getDisplayName(), result.time)));
            }
