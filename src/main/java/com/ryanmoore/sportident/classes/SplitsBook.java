...
public class SplitsBook {
    ...
    public SplitsBook(SportIdent SI, Player player, Result.Status status) {
        ...
        if (Main.config.showChipNumber) {
            addLine("Chip: " + (Main.config.minChipNumber
                    + (int) (Math.random() * ((Main.config.maxChipNumber - Main.config.minChipNumber) + 1))));
        }
        ...
        addLine(String.format("%1$-7s", "Start")
                + String.format(" %s %s", formatDuration(Duration.ofMillis(0)), formatDuration(Duration.ofMillis(0))));
